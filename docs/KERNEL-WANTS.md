...
> ### Corrected the same day, and then corrected again
>
> **The note that stood here was wrong, and how it went wrong is worth more
> than the answer.**
>
> It said: the probe named three functions, walk the call graph from them,
> find no `stat`, therefore `stat` is not what stands between the kernel and a
> screen with a window on it. Every step of that was sound. The premise was
> not. Those three were the symbols one particular link left *unresolved* --
> not the entry points a desktop uses. `recon_theme.c` and `recon_fonts.c`
> between them call **twelve** of this file's entry points: `read`, `write`,
> `append`, `list`, `exists`, `remove`, `copy`, `mkdir`, `join`, `resolve`,
> `unique_name`, `last_error`. Several reach `stat`.
>
> A correct walk from a false premise is a confident wrong answer, and this one
> reached the change log, the board and a note to the kernel session before an
> instrument contradicted it.
>
> ### What is true, from a linker rather than from an argument
>
> `scripts/link-desktop.sh` builds a program that reads its theme off the disk,
> fills a screen, frames a window, writes text in it and makes a directory --
> against every desktop source that compiles freestanding, the ReconOS C
> library, and **nothing else**.
>
> | | |
> |---|---|
> | Unresolved symbols | **one: `stat`** |
> | With `stat` stubbed | **links, 146,768 bytes, zero undefined** |
>
> So `stat` is the wall, and it is now the *last* wall rather than one item in
> a list of groups. That is a sharper thing to know than either of the two
> claims it replaces, and it is what the kernel should build next.
>
> `lstat` comes with it. `fstat` is not reachable from anything a desktop does.
>
...
> must resolve too, and a program that draws would need `rename`, `unlink`,
> `chmod` and the rest of a file manager. With them it needs what it uses.
>
> `scripts/fs-reachable.py` still explains *which* paths reach `stat` and is
> useful for that. It takes its entry points on the command line now, because

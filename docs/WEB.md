...
| **Response compression** | **built** | `Content-Encoding: gzip` — `server/http/deflate.c`, 55 checks, chosen from `Accept-Encoding` and applied by the server rather than by each handler. Text and JSON only, above 256 bytes, and only when the result is actually smaller. **Files are not compressed**: they go through the streaming path and this compressor has no incremental form. On the machine, the JSON log 3818 — 815 bytes. 0.29.0 |

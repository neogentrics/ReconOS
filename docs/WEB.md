...
| **Response compression** | **built** | `Content-Encoding: gzip` — `server/http/deflate.c`, 81 checks, chosen from `Accept-Encoding` and applied by the server rather than by each handler. Text and JSON only, above 256 bytes, and only when the result is actually smaller. Whole responses since 0.29.0; on the machine, the JSON log 3818 → 815 bytes. |
| **Streaming compression** | **built** | The same encoder in an incremental form — `deflate_stream_begin`/`write`/`end`, a 32 KiB window that slides, one deflate block spanning the whole body. Wired into `http_stream_begin`, so **files are compressed too**, which they were not before. Forced chunked, because the length on the wire is not known when the head goes out. **Never on a 206**: `Content-Range` counts the resource's bytes and a compressed range names nothing. On the machine, `/console.css` 687 → 421 bytes, decompressed by Python's `gzip` and compared byte for byte. 0.37.0 |
...
| **CGI-style external programs** | **blocked** | nothing in user mode can start a program — `KERNEL-WANTS.md` carries the entry. [needs: kw-start-program] |
| **FastCGI / a persistent app backend** | **blocked** | same, plus a socket to talk to it over. [needs: kw-start-program] |
| **Reverse proxy to another machine** | **built, not yet on the machine** | `server/http/proxy.c`, 39 checks. Hop-by-hop headers stop at it (RFC 7230 6.1), the body is forwarded by length and never re-framed, every wait is bounded, and an unreachable upstream is 502 rather than 500 — this server has not failed. The suite reads the request the *upstream* received, from a fake upstream that records it, because asserting on what this server meant to send is asserting on its own opinion of itself. **Two stated limits:** no `X-Forwarded-For` (`accept` does not report who connected), and the *normalised* target is forwarded, so `/a%2Fb` is proxied as `/a/b` — `request.c` keeps no raw copy and the information is gone before the proxy is called. Host suites only; the row says so because this board's legend means running and tested. 0.39.0 |
...
| **Conditional requests** | **built** | `ETag` on a read, 304 to a matching `If-None-Match`, and **412** to a stale `If-Match` on a write — so two programs can share a machine without one losing a change it never knew it made. The server answers the conditional read; a handler only says what its body is worth as a validator. 0.31.0 |
...
| `Host`-based virtual hosts | **built, and configurable** | since 0.25.0 the sites come from `/System/server.conf` rather than from the source — a virtual host you cannot configure is a code path with a suite. A configured site serves files under a name; handlers stay compiled in, because a file cannot name a C function that was not linked. A site carries a name and a `next`; the first whose name matches answers, one with no name claims everything, and a name nobody claims gets **421** rather than 404 -- the resource may exist, and this is not the machine that has it. Picked per request rather than per connection, because a keep-alive client may ask two sites down one socket. `http_host_matches` ignores the port and a trailing dot, and keeps a bracketed IPv6 literal whole. 0.24.0. |

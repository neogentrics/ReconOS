...
| **Reverse proxy to another machine** | **unblocked, not built** | `connect` works. The previous note here said it *answers `SYS_OK` for a closed port and returns before the handshake*, which was true of an older kernel and wrong about the current one: measured 21 September, a closed port answers **refused in 1 ms** and a real listener **ready in 1 ms**. `server/dial.c` is the client half. VF-045 |

...
- **Found in** `scripts/make-issues.py`, on 20 September 2026, by the network
  session, within an hour of writing the check that had it — by the check
  reporting a failure that was not true.
- **Was** `check_versions` read released versions from `git log -p --follow --
  kernel/Makefile` and matched the lines a diff **adds**. That part was right
  and is documented. What it missed is that **`git log -p` shows no diff at all
  for a merge commit** unless asked, so any version first written *by* a merge
  was invisible and read as never released.
- **On this project that is the common case rather than an edge.** Two branches
  each carry a `VERSION` line, the merge picks the number for the union, and
  that number is first written by the merge itself. It is how 0.5.4, 0.5.6 and
  0.5.8 were all set on this branch.
- **Found by** the check reporting *"0.5.4 is named by an entry and was never
  released"* while `a37297a` — a merge commit on this very branch — had set
  exactly that. The first suspicion was that the merge simply had not been
  committed yet, which was plausible and wrong; `git show a37297a` printing no
  diff for the file, and `git rev-list --parents` reporting two parents, is what
  settled it.
- **What it would have cost.** False failures, which are worse than no check:
  the register would have been reported broken over versions that were fine, and
  the first two or three of those would have been investigated before somebody
  learned to ignore the line. A check that cries wolf gets muted, and then it is
  not there for the real one.
- **Fixed in** kernel 0.5.9 on `network`, by `--diff-merges=first-parent`.
  First-parent rather than the full combined diff on purpose: the question is
  what this branch's history *set*, not what each side of a merge proposed.
- **Proved both ways.** Before the flag, `--check` reported 0.5.4 missing and
  exited 1. After it, *"16 version(s) named by entries, 0 not released"* and
  exit 0. The failing case was the live register rather than a contrived one,
  which is the only reason it was caught at all.
- **This is the second bug in this check in one day**, after the pattern that
  swallowed a full stop and the pattern that matched nothing. All three share a
  shape: **the check was wrong about where its evidence lived, not about what to
  do with it.** A checker is a claim about the world and needs testing against
  the world, not against its own reasoning.

...
| A configuration file | server | **built, and unrepairable** | `server/config.c` — the machine’s name, port, resolver, clock and sites, read at boot from `/System/server.conf`. The whole file is taken or none of it is, and the console names the line it refused. **Written once and never again:** nothing in user mode can replace or remove a file, so the server can leave a template and nothing can edit it. `docs/KERNEL-WANTS.md` carries it |

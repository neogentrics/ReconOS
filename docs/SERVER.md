...
|---|---|---|---|
| Web / API server | server | **built** | `server/http/` — several connections at once, name-based virtual hosts, writes guarded, running on the machine. **The check count deliberately is not repeated here**: this row carried *736 across eighteen suites* for four versions after both numbers stopped being true, and `server/README.md` is the one place a script compares against the run (VF-024), and gzip on the way out since 0.29.0 |
| Name-based virtual hosts | server | **built** | one listener, a chain of sites, the first whose name matches answers; a name nobody claims gets 421. Configured in `/System/server.conf` since 0.25.0, and measured on the machine: two names, two different documents. See `docs/WEB.md` §4 |
...
| Reverse proxy | server | **built, not yet on the machine** | `server/http/proxy.c` — 39 checks, and the design is mostly what a proxy must not do: hop-by-hop headers stop at it, the body is forwarded by length and never re-framed, and a client cannot reach past this server into a connection it cannot see. Unblocked yesterday when KF-257 turned out to be a wrong diagnosis; this row claimed *unblocked* once before on a wrong reading and `scripts/check-board.py` moved it to blocked the same day. Host suites only, and the row says so |

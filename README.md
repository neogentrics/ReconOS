...
[![bugs](https://img.shields.io/badge/bugs_recorded-363-da3633?style=flat-square)](docs/BUGS.md)
...
| **Display** | Modes it sets itself, behind one interface with **four backends**: the Bochs/VBE adapter, virtio-gpu over both the PCI and memory-mapped transports, and — identified, keeping the mode firmware set — **Intel Gen9** and **AMD RDNA2**, the latter written against the two adapters in this project's own desktop. Gen9 also has its register map and its mode arithmetic checked against known answers on every boot, with the modeset itself named step by step rather than attempted, because none of it can be exercised under QEMU and the one machine that has the silicon has no serial port. A framebuffer console, and `/dev/fb0` for a program to map. Each backend was written to disagree with the last; [docs/SIGNALS.md](docs/SIGNALS.md) has what each disagreement found |

...
def check_versions(text):
    ...
    **The kernel session's rule, mechanised**: *no entry may name a version
    ...
    `git log -p` shows **no diff for a merge commit** by default, and on this
    project the version is very often resolved *inside* a merge: two branches
    each carry a `VERSION` line, the merge picks the number for the union, and
    that number is first written by the merge itself.

    Without this flag every such version reads as *never released*. It was
    caught within the hour, by this check reporting 0.5.4 missing when 0.5.4 was
    set by `a37297a` -- a merge commit on this very branch.

    `first-parent` rather than the full combined diff: the question is what this
    branch's history set, not what each side proposed.
    """
    out = subprocess.run(['git', 'log', '-p', '--diff-merges=first-parent',
                          '--follow', '--', 'kernel/Makefile'],
